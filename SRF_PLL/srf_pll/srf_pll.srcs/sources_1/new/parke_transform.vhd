...
BEGIN
    ...
    BEGIN
        ...
        ELSIF rising_edge(clk) THEN
            v_d_int <= STD_LOGIC_VECTOR(
                           signed(v_alpha_delayed) * signed(cos_val) +
                           signed(v_beta_delayed)  * signed(sin_val)
                       );
            v_q_int <= STD_LOGIC_VECTOR(
                           signed(v_beta_delayed)  * signed(cos_val) -
                           signed(v_alpha_delayed) * signed(sin_val)
                       );
        END IF;
    END PROCESS;
