        ...
        END IF;
           ...
           FOR i IN 0 TO ITERATIONS-2 LOOP 
                    IF(pipeline(i).pip_theta < 0) THEN                               
                        pipeline(i+1).pip_cos <= pipeline(i).pip_cos + shift_right(signed(pipeline(i).pip_sin),i);
                        pipeline(i+1).pip_sin <= pipeline(i).pip_sin - shift_right(signed(pipeline(i).pip_cos),i);
